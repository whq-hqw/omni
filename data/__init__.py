...
def create_batch_from_queue(args, input_queue, output_shape, functions,
    ...
    output_batch = tf.train.batch_join(cell, batch_size=args.batch_size,
                                       capacity=32 * args.batch_size * args.threads,
                                       shapes=output_shape, enqueue_many=True,
                                       allow_smaller_final_batch=True)
    return output_batch


def load_images(args, paths, **options):
    ...
    for path in tf.unstack(paths):
        ...
        if args.random_brightness:
            image = tf.image.random_brightness(image, max_delta=args.imgaug_max_delta, seed=seed)
        # TODO: random contrast, random blur, crop_bbox
        if args.random_crop:
            image = tf.random_crop(image, [args.img_size, args.img_size, 3], seed=seed)
        else:
            image = tf.image.resize_image_with_crop_or_pad(image, args.img_size, args.img_size, seed=seed)
        if args.random_flip:
        ...
        if args.random_noise:
            ...
            noise = tf.add(tf.random_normal(shape=tf.shape(image), mean=args.imgaug_mean,
                                            stddev=args.imgaug_stddev, seed=seed), 1)
...
def pass_it(args, input, **options):
    # TODO: maybe we can use **options here to accept more parameters for flexibility
    #data = []
